"""Run after .env + schema setup. --ai makes one small paid OpenAI API request."""
import argparse
import asyncio
...
async def main():
    ...
    if args.ai or args.report_ai:
        if not config.OPENAI_API_KEY:raise ValueError('OPENAI_API_KEY is required')
        ai=AIAnalyzer(config.OPENAI_API_KEY,config.OPENAI_MODEL)
        try:
            result=await ai._json('ตอบ JSON เท่านั้น: {"ok":true}',{})
            if result!={'ok':True}:raise RuntimeError('Unexpected AI response')
            print('PASS: '+config.OPENAI_MODEL+' live response')
            if args.report_ai:
                ...
                await ai.normalize_batch(rows)
                print('PASS: normalization and exact evidence validation')
